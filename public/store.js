...
var stripeHandler = StripeCheckout.configure({ //uses the Stripe library
    key: stripePublicKey, //gets the publickey from the front end
    locale: 'auto', //what lanaguage it should use
    token: function (token) {
        ...
        fetch('/purchase', { //route on the server
            ...
            headers: {
                'Content-Type': 'application/json',//this is telling the server it will be receiving json information
            ...
            body: JSON.stringify({ //these are the items fetching to the server
                stripeTokenId: token.id, // sends the token ID to the server
                items: items //the items from the json file.
            })
